                        ...
                        <div className='grid grid-cols-1 w-full gap-3'>
                            ...
                            {selectedRooms.map((selectedRoom, index) => (<div key={index} className='grid grid-cols-3 w-full items-center justify-between text-[#636363] text-sm gap-1'>
                                <div className="flex gap-1 items-center">
                                    <Button variant={1} onClick={() => handleRemoveRoom(selectedRoom)}>X</Button>
                                    <span className='text-sm font-normal leading-6 '>{selectedRoom.roomNumber}</span>
                                </div>
                                <span className='text-sm font-normal leading-6 '>{numberOfDays}</span>
                                <span className='text-sm font-normal leading-6 '>NGN {Number(selectedRoom.roomType.price).toLocaleString()} </span>
                            </div>))}

                            <div className='grid grid-cols-3 w-full items-center justify-between text-sm gap-1'>
